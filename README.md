...
Setelah registrasi berhasil, backend membuat QR PNG yang hanya berisi `submissionId` UUID (tanpa NIK atau data pribadi). Respons `submitPayroll` menyediakan `submissionId`, `qrPayload`, `qrCodeUrl`, `qrCodeImageUrl`, `qrCodeDownloadUrl`, dan `qrCodeDataUrl` untuk integrasi dengan sistem absensi. Frontend memakai `qrCodeDataUrl` agar tampilan dan unduhan QR tidak bergantung pada izin hotlink Google Drive. Jika `QR_FOLDER_ID` tidak diisi, fungsi `setup()` otomatis membuat folder `QR_CODES` di folder induk KTP.
